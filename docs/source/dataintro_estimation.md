...
データから得られた平均購入金額は、男性が約438円、女性が約08円でした。
...
`gender`変数でフィルタをかけ、男女別に`sales`変数を分割し、`AVERAGE`関数を使って、男女それぞれの平均売上金額を計算すればそれでOKです。
...
[ノンパラメトリック回帰の注意点])(#ノンパラメトリック回帰の注意点)に挙げたような問題点に対処するために、説明変数$X$と被説明変数$Y$の関係になんらかの関数形を仮定する方法が、パラメトリック回帰です。

特に、被説明変数$Y$は説明変数$X$の1次式であると仮定する方法を線形回帰（linear regression）といいます。

例えば、売上金額（`sales`）は年齢（`age`）の1次式であると仮定すると、売上金額は以下の式で表すことができます。

$$
sales = \beta_{0} + \beta_{1} \times age + \varepsilon
$$

例のように、説明変数が一つのときには特に単回帰モデルといい、説明変数が二つ以上の時には重回帰モデルといいます。

ここで、回帰モデルについていくつかの専門用語を整理しておきます。

$Y = \beta_{0} + \beta_{1} X + \varepsilon$ のような説明変数$X$と被説明変数$Y$の関係式を回帰式と呼び、直線の切片$\beta_{0}$、直線の傾き$\beta_{1}$を回帰係数と呼びます。

線形回帰は、手元のデータ$(age, sales)$の組み合わせから、データの動きを最もよく表す直線の回帰係数$(\beta_{0}, \beta_{1})$を見つける作業です。
...
しかし、傾きが$0$ではないかどうかを確認したいときには、グラフだけでは不十分です。
...
傾きが$0$ではないかどうかを確認したいとき、説明変数$X$が2つ以上の場合は、[仮説検定](#仮説検定)でも使った分析ツールを使います。
...
2. 「回帰分析」を選択
3. 入力$Y$範囲に被説明変数、入力$X$範囲に説明変数のデータ範囲を入力
4. 1行目に変数名が入っているときにはラベルにチェックを入れる
5. 出力オプションに結果を出力したい場所を入力する
    - 一覧の入力先：入力した既存ワークシートのセルに出力される
    - 新規ワークシート：自動的に出力用の新規ワークシートが作成される
    - 新規ブック：自動的に出力用の新規ブックが作成される
