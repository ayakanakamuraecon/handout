...
（例）`sales`変数の平均：`=AVERAGE(:)` 

平均を計算することで、データの中心（重心）を知ることができました。学食の売り上げデータでは、1回あたりの購入金額は、**円のようです。
...
（例）`sales`変数の度数分布表から平均を求める：`=SUMPRODUCT(:)` 
...
ケース1では全員が同じ金額で商品（恐らく*商品名*でしょう）を購入しているため、売上を伸ばすためには、*商品名*を広告の全面に打ち出したり、*商品名*が売り切れないよう仕入れを増やすことが考えられるかもしれません。

一方ケース2では全員がバラバラの商品を購入しているため、*商品名*の販促だけに注力していると、売上が下がってしまうかもしれません。
...
（例）`sales`変数の分散：`=STDEV.P(:)` 

分散と標準偏差を求めることで、データのばらつきを知ることができました。学食の売り上げデータでは、購入金額は平均**円、平均から離れているようです。
...
上図は、売上金額と購入時間のデータを並べたものですが、生のデータを一見しただけでは、両者にどのような関係があるのか（あるいはないのか）を読み取ることは難しいと思います。
...
具体的に、売上金額と時間の関係を考えてみましょう。時間を横軸、売上金額を縦軸にとったとき、手元にある購入ID $i$ のデータは $(時間, 売上金額) = (x_{i}, y_{i})$ と表すことができます。この $(x_{i}, y_{i})$ を座標とする点を平面上に表したものが散布図です。
...
（例）`sales`変数と`time`変数の共分散：`=COVARIANCE.P(:, :)` 

共分散を計算することで、2変数の相関の強さを知ることができました。売上金額と時間の共分散は**、つまり、時間と売上の間には正の相関があるようです。

**共分散を解釈するときは単位に気をつけてください。**

共分散の計算式を見ると分かるように、**共分散の単位はそれぞれの変数の単位の積**になります。`sales`変数と`time`変数の共分散の場合、その**単位は円 $\times$ 分**です。

「時間が〇分経つと△円売上が上がる」といった解釈はできません。**共分散から分かるのは**、あくまでその**2変数間に正負どちらの相関があるのか**、だけです。

また、例えば`sales`変数と`date`変数の共分散が**だと分かった場合でも、`time`変数と`date`変数の**どちらが`sales`変数との相関が強いかを比較することはできません**。
...
学食データの売上金額と時間変数の相関係数を計算してみましょう。Excelでは`CORREL`関数を使います。
...
売上金額と時間の相関係数は**、つまり、時間と売上の間には弱い正の相関があるようです。

### 無相関 $\ne$ 関係がない

**「無相関であること」と「関係がない」は違う**意味を表します。

先ほど`sales`変数と`time`変数の相関係数は**であることが分かりました。基準に照らし合わせると2変数間にほとんど相関はないと言えます。
...
散布図は、**明らかに2変数が連動して動いている**ことを示しています。

**共分散や相関係数は、2変数間の線形的な（直線で示せる）相関関係のみを示し**ます。

今回のように`time`変数の1か所でピークがくるような、**線形以外の相関関係を表すことはできません**。
