...
データの中心を表す統計量としてよく使われるのは**平均**と**中央値**です。

2つの統計量はよく似ているように思われがちですが、それぞれに特徴や注意点があり、使い分けが必要です。
...
**中央値**（median）は、**データを小さい順に並べたときに真ん中に位置するデータの値**のことです。

例えば、$\{4, 1, 3, 2, 5\}$というデータを考えてみましょう。
5つのデータを小さい順に並べると、$\{1, 2, 3, 4, 5\}$となります。
このとき、3は小さい方から数えて3番目かつ大きい方から数えて3番目という、真ん中に位置するデータなので、このときの中央値は$3$になります。

次に、$\{4, 1, 3, 6, 2, 5\}$というデータを考えてみましょう。
6つのデータを小さい順に並べると、$\{1, 2, 3, 4, 5, 6\}$となります。
このとき、ちょうど真ん中に位置するデータはありません。
