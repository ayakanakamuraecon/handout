...
- [x] labor_intro.qmd内の3つの画像を、実データに基づくインタラクティブなSVG+JSグラフに作り直し（すべて`{{< include *.html >}}`で埋め込み、`<details>`に表形式フォールバックつき）。ユーザーとのブラウザ確認を重ねてフォントサイズ・線の太さ・国の絞り込み・ラベル配置・配色を調整。詳細な調整の経緯は`git log`参照（2026-08-13）
  - `workhour_chart.html`：「労働時間」節の`images/labor_workhour.png`（週労働時間の国際比較、JILPT表6-2）を置き換え。数値はユーザーと1行ずつ確認しながら書き起こし。最終的に対比の弱い国を除いて日本/アメリカ/ドイツ/スウェーデン/オランダ（欧米）＋韓国/中国/シンガポール/タイ/オーストラリア（アジア大洋州）の10か国に整理。日本は地理的にアジアのため単独の暖色（赤）に。ラベルは「右端に縦積み＋早期終了する国は点線リーダー」方式（ユーザー指定）。
  - `workforce_oecd_chart.html`：「年齢階級別労働力率」節2枚目の画像`images/labor_workforce_oecd.png`を置き換え。数値はJILPT公式サイトの関連表「第2-11-2表 性別・年齢階級別労働力率」（`https://www.jil.go.jp/kokunai/statistics/databook/2024/02/d2024_2T-11-2.pdf`）から取得。元画像の「日本（1975）」比較線はデータ入手不可のため削除。イギリス・ドイツは他国と類似パターンのためユーザー判断で削除し、最終的に日本/アメリカ/フランス/イタリア/スウェーデン/韓国の6か国。ラベルは「他系列との差が最大の点」を候補に線・ラベルとの衝突を避けて自動配置しつつ、複数か国はユーザー指定で手動固定（自動配置は最終的にフランスのみ）。
  - `unpaidwork_chart.html`：「労働時間」節の`images/labor_unpaidwork.gif`（男女別生活時間の国際比較、内閣府男女共同参画局、棒+比率折れ線の複合グラフ）を積み上げ棒グラフに置き換え。元画像は14か国比較だが、正確な数値が確認できたのは内閣府男女共同参画局白書コラム1本文（`https://www.gender.go.jp/about_danjo/whitepaper/r02/zentai/html/column/clm_01.html`）に明記された日本・OECD平均のみのため、この2グループに限定。男女比（有償=男/女、無償=女/男）は色分けした数値ラベルで直接表示（二軸グラフは不使用）。
  - 上記グラフの内容変更に合わせ、直後の本文（「図から分かること」、「役割分担型」「共働き分担型」カード、確認クイズ）も更新済み。
  - 色は`dataviz`スキルの検証済みカテゴリカルパレット・シーケンシャルランプを流用。このPCにNode.js/Pythonが無く`scripts/validate_palette.js`による公式検証は未実施（青系等の既存パレット値はそのまま使用、独自に類推構成した色もあり）。
  - この一連の作業中、render時にDropboxロックで`labor_intro_files`空フォルダの削除に失敗しoutputが更新されないことが再発した。毎回「出力に反映されたか」を`grep`で確認し、されていなければ空フォルダを消して再renderする運用を継続（環境メモ参照）。
- [x] `.callout-note`等の定義ボックスのタイトル（「賃金率」「労働力率」等）が本文と同じ大きさ(1.0625rem)で見出しとして小さく見える問題を修正。h4見出しと同じ1.1875remに拡大。`.callout-title-container`にも明示的に指定（`seminar-theme.scss`はプロジェクト共通のため`quarto render`（引数なし）で全体反映）（2026-08-13）
- [x] ここまでのローカルコミット（`ff9d0f1`〜`022ac5c`、計5コミット）を`origin/main`へpush完了（2026-08-13、家PC）
- [ ] labor_supdem.qmdの作業に着手（2026-08-13、ユーザー指示で次の作業対象に。labor_intro.qmdで確立した「画像→実データに基づくインタラクティブグラフへの置き換え」パターンを他ファイルにも展開する想定だが、具体的な範囲・対象箇所は未指定・要ユーザー確認）
