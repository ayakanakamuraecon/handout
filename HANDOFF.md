...
- [x] 「労働時間」節の`images/labor_workhour.png`（週労働時間の国際比較、JILPT表6-2のスクリーンショット表）を、実データに基づく折れ線グラフに作り直し。新規`workhour_chart.html`（インラインSVG＋JS、`{{< include workhour_chart.html >}}`でlabor_intro.qmdに埋め込み）を作成。欧米=青系／アジア大洋州=橙系のグラデーションで色分けし、本文で繰り返し対比される日本・韓国を各系統の最濃色に。ホバーで線を強調しツールチップ表示、折れ線末尾に国名ラベル（重なり回避の縦位置調整つき）、`<details>`内に表形式のフォールバックも用意（2026-08-13、ローカルコミットのみ、未push、ユーザーのブラウザ確認待ち）
  - 表データは元画像をユーザーと1行ずつ確認しながら書き起こし（2021年列を最初のOCR読み取りで完全に見落としていた反省あり）。最終データは`workinghour.txt`としてユーザーから提供された内容を採用。
  - 色（橙系）は`dataviz`スキルの検証済みパレット（青系ランプ・カテゴリ色）を流用しつつ手動で類推構成したもの。このPCにNode.js/Pythonが無く`scripts/validate_palette.js`を実行できなかったため、公式の検証は未実施（青系は`references/palette.md`記載の検証済みシーケンシャルランプをそのまま使用）。
  - ユーザーのブラウザ確認を経て複数回調整（2026-08-13）：
    - フォント（軸ラベル・国名ラベル・ツールチップ）と線を全体的に太く/大きく
    - 対比があまり明確でない国を削減：香港・フィリピン・インドネシア・カナダ・イギリス・フランス・イタリア・ニュージーランドを除外し、日本/アメリカ/ドイツ/スウェーデン/オランダ（欧米5）＋韓国/中国/シンガポール/タイ/オーストラリア（アジア大洋州5）の計10か国に整理
    - 図上部の「欧米／アジア」凡例は削除（軸ラベル拡大＆残り10か国で色の意味は見た目からも分かりやすいと判断）
    - 日本は地理的にアジアのため青系ではなく単独の暖色（赤 `#e34948`）に変更。オランダは元々青系統でもっとも薄い色を割り当てていて見づらかったため、より視認性の高い青（`#86b6ef`）に変更
- [x] 「年齢階級別労働力率」節の2枚目の画像`images/labor_workforce_oecd.png`（年齢階級別女性労働力率の国際比較、JILPTのグラフ画像）も、実データに基づく折れ線グラフに作り直し。新規`workforce_oecd_chart.html`を作成し`{{< include workforce_oecd_chart.html >}}`で埋め込み（2026-08-13、ローカルコミットのみ、未push）
  - 元画像はグラフ（表ではない）で各点に数値が印字されていないため、まずJILPT公式サイトを検索し、関連表「第2-11-2表 性別・年齢階級別労働力率」（`https://www.jil.go.jp/kokunai/statistics/databook/2024/02/d2024_2T-11-2.pdf`）から女性・2022年の正確な数値を取得。日本/アメリカ/イギリス/ドイツ/フランス/イタリア/スウェーデン/韓国の8か国、年齢階級11区分（65+はJILPTの公式集計値をそのまま使用、65-69/70-74/75+の再集計はしていない）。
  - 元画像にあった「日本（1975）」の比較線は、当年版データブックに1975年データがなく、正確な値を再現できないためユーザー判断で削除（2022年の8か国国際比較のみに絞った）。
  - 色は日本=単独の赤`#e34948`（本文で焦点となる国のため）、他7か国は`dataviz`スキルの検証済みカテゴリカルパレット8色から赤を除いた7色を固定順で割り当て。
- [x] 両チャート（workhour_chart.html / workforce_oecd_chart.html）のラベル配置を改善。「右端に全部寄せる」方式は見づらいとユーザーから指摘があり、各系列について「他系列との差が最大の点（クリアランス最大点）」を候補に、既存ラベルと重ならない位置（上下左右8方向のオフセット）を貪欲法で探す方式に変更。位置がずれる場合は元の点まで点線でつなぐ。ラベルのフォントサイズも15px→17pxに拡大（2026-08-13）
- [x] 上記の「クリアランス最大点」方式は、ラベルが他系列の線の上に重なって読めなくなる不具合があった。workforce_oecd_chart.htmlはラベル候補ボックスが各系列の経路（線分を補間サンプリング）と重ならないことも判定条件に追加して解消。workhour_chart.htmlは、ユーザー指示によりこの方式自体を撤回し、元の「右端に縦積み＋早期終了する国は点線リーダー」方式に戻した（フォントサイズ17pxは維持）（2026-08-13）
- [x] workforce_oecd_chart.htmlのラベル配置を自動4か国＋手動4か国の2パス方式に変更。日本/イギリス/フランス/スウェーデンは自動配置のまま、韓国（35-39歳・40%あたり）/イタリア（自動候補点から2文字右）/ドイツ（スウェーデンの1文字左・真上）/アメリカ（線のない45-49歳・50%あたり）はユーザー指定の位置に手動固定。ドイツの位置はスウェーデンの自動配置結果を実行時に参照して計算（コード内`chosenByName`で解決）（2026-08-13）
- [x] さらに微調整：ドイツもう1文字上、アメリカ1文字右、日本1文字右上、フランス1文字上（`autoPlace`に`nudge`引数を追加して自動配置後にオフセット、手動配置の2件は直接座標を調整）（2026-08-13）
- [x] 「労働時間」節の`images/labor_unpaidwork.gif`（男女別生活時間の国際比較、内閣府男女共同参画局、棒グラフ+比率の折れ線という複合グラフ）も作り直し。新規`unpaidwork_chart.html`（積み上げ棒グラフ：有償労働+無償労働、男女比は数値ラベルで直接表示、二軸グラフは使わない）を作成し埋め込み（2026-08-13、ローカルコミットのみ、未push）
  - この元画像は14か国の国際比較だが、正確な数値が取得できたのは日本とOECD平均の2つのみ（後述）。ユーザー判断で日本・OECD平均の2グループに絞って作図。
  - データ取得の経緯：画像はJILPTの表と違い低解像度のgifかつ棒+折れ線の複合グラフで、印字数値の目視読み取りは信頼性が低いと判断。まず出典元の内閣府男女共同参画局サイト（`https://www.gender.go.jp/about_danjo/whitepaper/r02/zentai/html/column/clm_01.html`、令和2年版白書コラム1本文）を検索・取得し、本文中に明記されていた日本とOECD平均の数値（分/日）のみを採用。日本：有償労働 男性452分/女性272分、無償労働 男性41分/女性224分。OECD平均：有償労働 男性317分/女性218分、無償労働 男性136分/女性262分。
  - 別の候補としてOECD Family Database「LMF2.5: Time use for work, care and other day-to-day activities」（`https://webfs.oecd.org/Els-com/Family_Database/LMF2_5_Time_use_of_work_and_care.pdf`）も見つけたが、集計年代・分類方法が元図の出典「OECD (2020), Balancing paid work, unpaid work and leisure」と異なり、日本の数値を照合しても一致しなかったため不採用。
  - 直後の本文もグラフに合わせて修正済み（2026-08-13）：
    - 14か国前提の箇条書き（「日本とイタリアの1.7倍」「韓国の4.4倍」「フィンランドとスウェーデンで比率はほぼ1」等）を、`.seminar-tip`「図から分かること」（基本的に有償労働は男性・無償労働は女性が長い／有償労働の男女比率は日本とOECD平均で大差なし／無償労働の男女比率は日本の方がはるかに大きい／分担の仕方は国によって異なる、の4点）に置き換え
    - 「役割分担型」ボックスの書き出しを「日本や韓国のケースでは」→「日本のケースでは」
    - 「共働き分担型」ボックスから「フィンランドやスウェーデンのように」を削除
    - 確認クイズの問題文も「日本や韓国など」→「日本など」に統一
  - 「図から分かること」の4項目はユーザーの下書きそのままではなく、既存の他の箇条書きと文体を揃えて校正（「〜傾向にある。」「〜あまり差がない。」等の文末統一）
- [x] `.callout-note`等の定義ボックスのタイトル（「賃金率」「労働力率」等）が本文と同じ大きさ(1.0625rem)で見出しとして小さく見える問題を修正。h4見出しと同じ1.1875remに拡大。`.callout-title-container`にも明示的に指定（`seminar-theme.scss`はプロジェクト共通のため`quarto render`（引数なし）で全体反映）（2026-08-13、ローカルコミットのみ、未push）
- [x] unpaidwork_chart.htmlの軸ラベルフォント拡大（16px→19px、目盛りの14px→17px）。縦軸の目盛り「500」と単位ラベル「分/日」が重なっていた不具合を修正：単位ラベルをグラフ左上・目盛りの外側（`margin.top`を16→36に広げてその上の余白に配置）に移動（2026-08-13）
- [x] 男女比の表示を目立たせるよう変更。1行にまとめていた「有償 男/女=X倍　無償 女/男=Y倍」を2行に分割し、フォントサイズを本文と同じ17pxに拡大、有償労働は`--paid`（青）・無償労働は`--unpaid`（橙）で色分け（凡例と対応）。表示スペース確保のため`margin.bottom`を74→110、`height`を480→500に拡張（2026-08-13、ローカルコミットのみ、未push）
- [x] イギリスはドイツ/フランス/スウェーデンとほぼ同じ「高原型」パターンで値も近接しラベルが見づらいため、ユーザー判断で削除。国際比較は日本/アメリカ/ドイツ/フランス/イタリア/スウェーデン/韓国の7か国に（2026-08-13）
- [x] イギリス削除後、スウェーデンの自動配置結果（クリアランス最大点はデータセット全体に依存するため）が変わり、それに連動してドイツの位置もズレた。原因をユーザーに説明し、スウェーデンも韓国/イタリア/アメリカと同様に手動固定に変更（60-64歳と65+の間くらい・70%、さらに2文字右）。これでデータが変わっても位置が安定する。自動配置に残るのは日本・フランスのみ（2026-08-13）
- [x] ドイツとフランスはどちらも高原型で似ているためどちらか削除してもよいとユーザーから相談があり、フランスは60-64歳で急落する(38.7、ドイツの60.9より大幅に低い)早期引退傾向という特徴的な形があるため残し、ドイツを削除するよう提案・実施。国際比較は日本/アメリカ/フランス/イタリア/スウェーデン/韓国の6か国に。ドイツ依存だったドイツのラベル配置コードも削除（2026-08-13）
- [x] 日本とスウェーデンのラベルが重なっていたため、日本も自動配置から手動固定に変更（20-24歳と25-29歳の間くらい・90%）。自動配置に残るのはフランスのみ（2026-08-13、ローカルコミットのみ、未push）
  - この一連のラベル微調整中、render時にDropboxロックで`labor_intro_files`空フォルダの削除に失敗しoutputが更新されないことが再発した。毎回「出力に反映されたか」を`grep`で確認し、されていなければ空フォルダを消して再renderする運用を継続（[[環境メモ]]参照）
