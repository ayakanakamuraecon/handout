...
**最終更新**: 2026-08-28 / 家PC（THINKPAD-AURA）
...
- [ ] **上記すべて（見出しレベル整理＋ユーザー手動改訂2件＋大企業/中小企業カード化＋労基法改正カード化）をコミット・push**（未実施。下記の`git`不具合により本日は断念、次回の最初にやること）
...
- **【新規・未解決】`git status`/`git diff`/`git cat-file`/`git fsck`等、ほぼ全てのgitコマンドが`fatal: mmap failed: Invalid argument`で失敗する不具合が発生（2026-08-28、家PC）**。
  - 症状：`git log -1`・`git rev-parse HEAD`（コミットオブジェクトの読み取りのみ）は成功するが、`git status`・`git diff --cached`・`git cat-file -s HEAD:<path>`・`git fsck --full`は軒並み失敗。PowerShellとGit Bash（Bashツール）の両方で再現し、ツール固有の問題ではない。
  - 原因の推定：`.git/objects/pack/pack-dda99583d3ca94e0d169ce445d80afaacda6af97.pack`（唯一のpackファイル）が`ReparsePoint`属性を持っていることを確認（`fsutil reparsepoint query`でDropboxのクラウドファイルフィルタドライバのタグと判明）。`[System.IO.File]::ReadAllBytes()`による通常読み込みは成功する（＝データ自体はローカルにある）ため、Dropboxの新しい同期エンジンのreparse point機構と、gitがpackファイル読み込みに使うmmap（メモリマップドファイル）方式が根本的に非互換になっている可能性が高い。`.quarto`で過去に発生した「クラウド ファイル プロバイダー」問題（環境メモ参照）とは別種の、より深刻な不具合。
  - 試した対処（いずれも無効）：①Dropboxプロセスの再起動（`Stop-Process -Name Dropbox -Force`→再起動）→ 解消せず。②`.git`フォルダに`com.dropbox.ignored`属性を設定（`cmd /c "echo 1 > .git:com.dropbox.ignored"`、`.quarto`と同じ対処）→ 設定後もpackファイルの`ReparsePoint`属性は残ったままで、`git status`は依然失敗。既存ファイルには遡及的に効かない可能性がある。
  - 影響：labor_supdem.qmdの本日分の編集内容（見出しレベル整理・大企業/中小企業カード化・労基法改正カード化など、上記TODO参照）が**コミット・push未実施のまま**。`labor_supdem.qmd`・`HANDOFF.md`・`docs/labor_supdem.html`・`docs/search.json`の4ファイルは`git add`自体は個別には成功しており、変更内容そのものは失われていない（ワーキングツリー上に残っている）。
  - 次回試すとよいこと（未実施）：(a) Dropboxを完全終了した状態（サインアウトまたはアプリ終了）でgitコマンドが通るか確認する。(b) `fsutil reparsepoint delete`でpackファイルのreparse pointだけを除去できるか試す（データ消失リスクがあるため事前に`.git`ごとバックアップを取ってから）。(c) 最終手段として、別ドライブ（Dropbox配下でない場所、例: `C:\repos\`）に`git clone`し直し、ワーキングツリーの変更差分だけを`git diff`ではなく手動コピーで移植する。(d) Dropboxのアプリのバージョン・「同期エンジン」設定を確認する。
